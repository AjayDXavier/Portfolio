...
 const numClusterStars = 300;
 const clusterRadius = 20;

 const starData = useMemo(() => {
 const stars = [];
 const coreColor = new THREE.Color("#FFDDC4");
 for (let i = 0; i < numClusterStars; i++) {
 const pos = new THREE.Vector3()
 .randomDirection()
 .multiplyScalar(clusterRadius * (0.5 + Math.random() * 0.5));

 stars.push({ pos, color: coreColor });
 }
 return stars;
 }, []);
...
 return (
 <instancedMesh ref={meshRef} args={[null, null, numClusterStars]}>
 <sphereGeometry args={[0.4, 16, 16]} />
 <meshStandardMaterial emissiveIntensity={2} emissive="#FFDDC4" />
 </instancedMesh>
 );
}
...
 return (
 <instancedMesh ref={meshRef} args={[null, null, numStars]} rotation={[-Math.PI / -6.3, 2, 0]}>
 <sphereGeometry args={[0.6, 16, 16]} />
 {/* THIS IS THE CORRECTED MATERIAL */}
 <meshStandardMaterial vertexColors emissive="cyan" emissiveIntensity={2} />
 </instancedMesh>
 );
}
...
 <pointLight position={[0, 0, 200]} intensity={2} color="#a00a0aff" />
 <Stars />
 <SphereCluster />
 </group>
 <EffectComposer>
